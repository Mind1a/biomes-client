...
const BiomeMap = () => {
  ...
  const activeBiomeRef = useRef<typeof activeBiome>(null);
    ...
    if (biome) {
      ...
      if (e) {
        ...
        if (rect) {
          ...
          const position: "top" | "bottom" | "left-side" | "right-side" =
            isRightSide
              ? "right-side"
              : isLeftSide
                ? "left-side"
        ...
        if (mapContainer && biomePaths.length > 0) {
          ...
          const pathCenterX =
            pathRect.left + pathRect.width / 2 - containerRect.left;
          const pathCenterY =
            pathRect.top + pathRect.height / 2 - containerRect.top;
          ...
          const isLeftSide =
            pathCenterX > containerRect.width - popupHalfWidth - padding;
  ...
  useEffect(() => {
    const handleResize = () => {
      const current = activeBiomeRef.current;
      if (!current) return;

      const currentBiome = biomesInfo.find((b) => b.name === current.name);
      if (!currentBiome) return;

      const mapContainer = document.querySelector(".relative-container");
      const biomePaths = document.querySelectorAll(`#${currentBiome.id} path`);

      if (mapContainer && biomePaths.length > 0) {
        const containerRect = mapContainer.getBoundingClientRect();
        const randomIndex = Math.floor(Math.random() * biomePaths.length);
        const pathRect = biomePaths[randomIndex].getBoundingClientRect();
  ...
  return (
    <div className="flex gap-0 bg-[#eeeeee] font-sans text-[#1a1a1a] overflow-x-hidden">
      <div className="flex-1 flex flex-col min-w-0 md:p-4 w-full box-border max-w-348 mx-auto">
        <div
          className="relative after:absolute after:w-full after:h-full mt-10 after:top-0 after:bg-transparent md:after:hidden relative-container w-full h-auto max-w-331 mx-auto [&_svg]:w-full [&_svg]:h-auto [&_svg]:block
[&_g[id]_path]:stroke-[0.5px]
  [&_g[id]_path]:transition-opacity [&_g[id]_path]:duration-200"
        >
          <Map
            onBiomeSelect={(id, e) => handleBiomeSelect(id, e)}
            selectedBiomes={selectedBiomes}
          />

          {activeBiome && (
            <PopUp
              selectBiomeId={activeBiome.id}
          ...
          <div className="sm:hidden absolute right-4 z-50">
            {activeBiome && (
              <Link
                href={`/biomes/${activeBiome.id}`}
                className="flex items-center gap-2 border border-[#E0E0E0] bg-white rounded-xl p-3"
              >
                <span className="flex items-center gap-2 leading-7.5 text-[13px] font-semibold">
                  ბიომის გვერდი
                  <Image
                    width={100}
                    height={100}
                    src="/icons/mobile_link_iconr.svg"
                    alt=""
                    className="w-3 h-3"
                  />
                </span>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center mt-5 w-full max-w-full box-border">
          <div className="py-2 pr-4 pl-4 lg:border-t lg:border-r border-gray-500 shrink-0">
            ...
            >
              <span
                className={`w-2 h-2 rounded-[100px] ${selectedBiomes.length < allIds.length ? "bg-white opacity-70" : "bg-[#E6E6E6]"}`}
              ></span>
              <span
                className={`${selectedBiomes.length < allIds.length ? "text-[#666666]" : "text-white"} text-[12px] pt-1.5`}
              >
                ყველა
              </span>
            </button>
          ...
          >
              ...
              {biomesInfo.map((biome) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      backgroundColor: isSelectedBiome
                        ? `${biome.color}20`
                        : "white",
                      border: isSelectedBiome
                        ? `1px solid ${biome.color}`
                        : "1px solid #A8A8A8",
                    }}
                  ...
                  >
                    ...
                    <span className="text-start lg:text-nowrap">
                      {biome.name}
                    </span>
                  </button>
                );
            ...
            {scrollPosstion.right && (
              <button
                ...
                className="absolute right-1.5 top-1/2 -translate-y-1/2 bg-white rounded-full w-7 h-7 hidden lg:flex items-center justify-center cursor-pointer outline-none z-50 border-2 border-gray-100 shadow-md"
              >
                <Image
                  width={100}
                  height={100}
                  src="/assets/icons/biomesIcons/arrow.svg"
                  className="w-3 h-3 rotate-0"
                  alt=""
                />
              </button>
            )}
